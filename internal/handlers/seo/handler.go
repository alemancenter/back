...
// OptimizeAndSave is a retired compatibility endpoint. Generation and publishing
// must be separate requests so the editor sees the suggested fields before saving.
func (h *Handler) OptimizeAndSave(c *fiber.Ctx) error {
	...
	return c.Status(fiber.StatusConflict).JSON(fiber.Map{
		"success": false,
		"message": "افتح محرر المحتوى، راجع اقتراح SEO، ثم احفظه يدويًا",
	})
}
