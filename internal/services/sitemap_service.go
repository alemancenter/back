...
func (s *sitemapService) GetStatus(dbCode string) map[string]SitemapInfo {
	types := []string{"articles", "post", "static", "download", "index"}
	baseURL := s.siteURL()
...
func (s *sitemapService) GenerateAll(dbCode string) []error {
	...
	// Only files whose parent article/post is publicly indexable are included.
	wg.Add(1)
	go func() {
		defer wg.Done()

		rows, err := s.repo.GetIndexableDownloads(dbCode)
		if err != nil {
			errs[3] = err
			return
		}
		...
		for _, r := range rows {
			set.URLs = append(set.URLs, urlEntry{
				Loc:        fmt.Sprintf("%s/%s/download/%d", base, cc, r.ID),
				LastMod:    r.UpdatedAt.UTC().Format(time.RFC3339),
				ChangeFreq: "monthly",
				Priority:   "0.6",
			})
		...
		errs[3] = s.writeXML(
			s.sitemapFilename("download", dbCode),
			set,
		)
	}()
	...
	if errs[0] == nil &&
		...
		errs[2] == nil &&
		errs[3] == nil {
		errs[4] = s.writeSitemapIndex(
			dbCode,
			base,
			[]string{"articles", "post", "static", "download"},
		)
	}
