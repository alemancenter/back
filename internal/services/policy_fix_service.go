...
// content-policy problem — thin/medium content, near-duplicate content, corrupted
// unresolved-template artifacts, a too-short title, or a missing/short meta description —
// the same problems the adsense-policy dashboard page (internal/handlers/adsensepolicy) surfaces
// from EvaluateDiagnostics/DetectSimilarity/DetectReplacementArtifacts. It reuses that exact
// scoring so the fix is judged by the same bar the dashboard warning came from, reuses
// generateSEODraft unchanged for the title/meta-description pass, and — like GenerateDraft —
// never writes to the database itself: the result is always a reviewable draft the admin applies
// from inside the normal article/post edit page before clicking the ordinary Save button.
package services
...
//     text smoothed;
//   - a too-short title and a too-short/missing meta description are both regenerated by the
//     existing, already-tested generateSEODraft pass (scored against the same AnalyzeSEO the
//     manual "تحليل الآن" button uses), scoped to the final content so the description stays
//     accurate to what's actually on the page — a Google Publisher Policies requirement, not
//     just a nicety.
func (s *contentDraftService) FixPolicyContent(ctx context.Context, req PolicyFixRequest) (*PolicyFixResult, error) {
	...
	needsContentFix := contentThin || contentMedium || dup != nil || len(artifacts) > 0
	...
	if needsContentFix || needsTitleFix || needsMetaFix {
		...
		if seo, seoWarning := s.generateSEODraft(ctx, seoReq, finalContentHTML); seo != nil {
			...
			// Regenerate the meta description whenever it was itself flagged, OR whenever the
			// content changed — an unchanged meta description next to rewritten content risks
			// becoming inaccurate, and Google Publisher Policies require metadata that honestly
			// reflects the page's actual content.
			if needsMetaFix || needsContentFix {
				if needsMetaFix {
...
func (s *contentDraftService) fixContentWithRetries(ctx context.Context, req PolicyFixRequest, candidateKey string, fixCtx contentFixContext, excludeArticleID, excludePostID uint64) (html string, dup *contentquality.DuplicateMatch, filler []string, warning string) {
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		attemptFiller := detectGenericFillerPhrases(attemptHTML)
